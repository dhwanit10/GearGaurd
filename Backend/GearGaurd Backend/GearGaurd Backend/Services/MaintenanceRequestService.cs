...
public class MaintenanceRequestService : IMaintenanceRequestService
{
    ...
    public async Task<MaintenanceRequestResponseDto?> CreateMaintenanceRequestAsync(CreateMaintenanceRequestDto request)
    {
        ...
        if (teamMemberIds.Any())
        {
            ...
            if (availabilityToDelete != null)
            {
                ...
                scheduledDate = DateTime.UtcNow; // Set to today if assigned
            }
        }
        ...
        var maintenanceRequest = new MaintenanceRequest
        {
            ...
            ScheduledDate = scheduledDate, // Set to today if assigned, null otherwise
    ...
    public async Task<MaintenanceRequestResponseDto?> UpdateMaintenanceRequestAsync(UpdateMaintenanceRequestDto request)
    {
        ...
        string oldStatus = maintenanceRequest.Status;
        ...
        // If status changed to "Repaired", add maintenance person back to Availability table
        if (request.Status.Equals("Repaired", StringComparison.OrdinalIgnoreCase) && 
            !oldStatus.Equals("Repaired", StringComparison.OrdinalIgnoreCase) &&
            maintenanceRequest.MaintenancePersonId.HasValue)
        {
            // Check if availability already exists
            var existingAvailability = await _context.Availabilities
                .AnyAsync(a => a.TeamMemberId == maintenanceRequest.MaintenancePersonId.Value);

            if (!existingAvailability)
            {
                var availability = new Availability
                {
                    TeamMemberId = maintenanceRequest.MaintenancePersonId.Value
                };
